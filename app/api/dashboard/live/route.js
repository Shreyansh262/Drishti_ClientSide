    ...
    const [alcoholResult, visibilityResult, drowsinessResult, obdResult, historyResult] = await Promise.allSettled([
      ...
      (async () => {
        ...
        try {
          ...
          const twoDaysAgo = new Date(nowIst.getTime() - 2 * 24 * 60 * 60 * 1000).toISOString() + '+05:30';
          const sixHoursAgo = new Date(nowIst.getTime() - 6 * 60 * 60 * 1000).toISOString() + '+05:30';
          const thisMonth = nowIst.getUTCMonth();

          const last48Hours = all.filter(x => x.time >= twoDaysAgo && x.time <= nowIstIso);
          const recent = last48Hours.filter(x => x.time >= sixHoursAgo);
          ...
          let score = 100;
          if (recent.length === 0) {
            const hrs = Math.floor((new Date(nowIstIso.replace('+05:30', '')) - new Date(twoDaysAgo.replace('+05:30', ''))) / (60 * 60 * 1000));
          ...
          console.log(`History - Recent Incidents: ${recent.length}, Driver Score: ${driverScore}, Active Incidents: ${last48Hours.length}`);
          return {
